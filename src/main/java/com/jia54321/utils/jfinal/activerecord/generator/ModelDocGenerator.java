 ...
 *  model doc 生成器
 */
public class ModelDocGenerator {

	protected Engine engine;
	protected String template = "/com/jfinal/plugin/activerecord/generator/model_doc_template.jf";
	...
	public ModelDocGenerator(String baseModelPackageName, String baseModelOutputDir) {
	 ...
	 */
	public ModelDocGenerator setTemplate(String template) {
	...
	public void generate(List<TableMetaExtend> tableMetas) {
		...
		writeToFile(engine.getTemplate(template).renderToString(data));
	}


	protected void writeToFile(String out) {
		try {
			writeToFileByString(out);
		} catch (IOException e) {
	 ...
	 * base model 覆盖写入
	 */
	protected void writeToFileByString(String out) throws IOException {
		File dir = new File(baseModelOutputDir);
		if (!dir.exists()) {
			dir.mkdirs();
		}

		String target = baseModelOutputDir + File.separator  + "document.docx";
