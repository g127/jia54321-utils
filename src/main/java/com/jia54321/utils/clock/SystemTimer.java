...
public class SystemTimer {
	...
	public static void removeTimerListener() {
		if (list == null) {
			synchronized (TimerTask.class) {
				if (list == null) {
					list = new ArrayList<>();
	...
	private final static ScheduledExecutorService EXECUTOR = new ScheduledThreadPoolExecutor(1, new ThreadFactory() {
		...
		public Thread newThread(Runnable runnable) {
			Thread thread = new Thread(runnable, "Jia54321SystemTimer");
			thread.setDaemon(true);
			return thread;
		}
	});

	private static final long INITIALDELAY = Long.parseLong(System.getProperty("jia54321.system.timer.initialDelay", "10"));
	private static final long PERIOD = Long.parseLong(System.getProperty("jia54321.system.timer.period", "10"));

	public static volatile long currTime = System.currentTimeMillis();

	static {
		EXECUTOR.scheduleAtFixedRate(new TimerTask(), INITIALDELAY, PERIOD, TimeUnit.MILLISECONDS);
		Runtime.getRuntime().addShutdownHook(new Thread("Jia54321SystemTimer-Shutdown") {
			@Override
			public void run() {
				EXECUTOR.shutdown();
			}
